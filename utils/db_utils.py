...
def get_column_contents(column_name, table_name, cursor):
    """获取列的内容样本"""
    select_column_sql = "SELECT DISTINCT `{}` FROM `{}` WHERE `{}` IS NOT NULL LIMIT 2;".format(
...
def get_db_schema_sequence(schema):
    ...
    格式:
    database schema :
    table table_name , columns = [ table_name.column_name ( type | primary key | comment | values ) , ... ]
