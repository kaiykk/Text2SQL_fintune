...
def prepare_inputs_and_labels(sample, tokenizer):
    ...
    tokenized_inputs = tokenizer(
        input_words, return_tensors="pt", is_split_into_words=True,
    ...
    return encoder_input_ids, encoder_input_attention_mask, column_name_token_indices, table_name_token_indices, column_num_in_each_table
...
def split_sample(sample, tokenizer):
    ...
    for table in sample["schema"]["schema_items"]:
        table_names.append(table["table_name"] + " ( " + table["table_comment"] + " ) " if table["table_comment"] != "" else table["table_name"])
        column_names.append([column_name + " ( " + column_comment + " ) " if column_comment != "" else column_name for column_name, column_comment in zip(table["column_names"], table["column_comments"])])
    ...
    for table_idx, table_name in enumerate(table_names):
        for column_name in column_names[table_idx]:
            if get_sequence_length(text, recorded_tables_and_columns + [[table_name, column_name]], tokenizer) < 500:
                ...
                splitted_samples.append({"text": text, "schema": get_schema(recorded_tables_and_columns)})
...
def filter_schema(dataset, dataset_type, sic, num_top_k_tables=5, num_top_k_columns=5):
    """过滤 schema,保留最相关的表和列"""
    for data in tqdm(dataset, desc="filtering schema items"):
...
class SchemaItemClassifierInference:
    ...
    def predict_one(self, sample):
        encoder_input_ids, encoder_input_attention_mask, column_name_token_indices, table_name_token_indices, column_num_in_each_table = prepare_inputs_and_labels(sample, self.tokenizer)
